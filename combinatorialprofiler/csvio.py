...
def readBarcodes(f):
    reader, has_header = get_csv_reader(f)
    if has_header:
        next(reader)
...
def readCellCounts(f, barcodes_fw=None, barcodes_rev=None):
    ...
    if types['float64'] != df.columns.size - 1:
        ...
        if barcodes_fw is not None and len(barcodes_fw):
            ...
            if not set(df.index).isdisjoint(barcodes_rev):
                toreturn = df.T.stack()
