...
void ReadCounter::init()
{
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, NodeBase*>> fwNodes;
    std::unordered_map<std::string,NodeBase*> dummyNodes;

    for (const auto &exp : m_experiments) {
        std::vector<NodeBase*> revNodes;
        for (const auto &c : exp->revBarcodeSet) {
            NodeBase *n = makeRevCodeNode(exp, c.first);
            n->experiment = exp;
        ...
        for (const auto &c : exp->fwBarcodeSet) {
            if (!fwNodes[exp->insert].count(c.first)) {
                NodeBase *n = makeFwCodeNode(exp, c.first);
...
const std::vector<Experiment*>& ReadCounter::experiments() const
{
